...
> 최종 산출물 제출용 「팀 통합 Backlog / 개발 기여 현황」 문서.
> `backlog/` 폴더의 팀원별 기록(`sara.md`, `seungwoo.md`, `eunyeong.md`,
> `hyejune.md`, `aaron.md`)을 취합했다. 각 팀원의 원본 기록을 기준으로 하며,
> 원본에 없는 내용은 추가하지 않았다. 팀원 순서는 `02_TEAM_GUIDE.md §1`의 팀 표
> 순서를 따르며(사라 · 승우 · 은영 · 혜준 · 아론), 업무량 평가나 순위 부여의
> 의미는 없다.
...
> 상태는 원본 backlog의 `[QA]` 기록에서 확인되는 표현만 사용한다.
> **완료** = 원본에 "현재 정상 / 정상 / 검증 완료 / E2E 통과"로 기록된 경우.
> **보완 필요** = 원본에 "수정 필요 / 재확인 필요 / 재검증 필요 / 추가 개발 필요"로
> 기록된 경우. 원본에 상태 표현이 없으면 **미표기**.
...
> `backlog/` 폴더의 기존 5개 파일은 수정하지 않았다.
